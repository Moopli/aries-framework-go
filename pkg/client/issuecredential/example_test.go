...
func ExampleClient_SendOffer() {
	...
	_, err = clientAlice.SendOffer(&OfferCredential{}, &service.ConnectionRecord{MyDID: Alice, TheirDID: Bob})
	if err != nil {
		fmt.Println(err)
	}
...
func ExampleClient_SendProposal() {
	...
	_, err = clientBob.SendProposal(&ProposeCredential{}, &service.ConnectionRecord{MyDID: Bob, TheirDID: Alice})
	if err != nil {
		fmt.Println(err)
	}
...
func ExampleClient_SendRequest() {
	...
	_, err = clientBob.SendRequest(&RequestCredential{}, &service.ConnectionRecord{MyDID: Bob, TheirDID: Alice})
	if err != nil {
		fmt.Println(err)
	}
